...
export default function LanguageSwitcher() {
  ...
  const select = (lang: typeof languages[0]) => {
    ...
    const isProd =
      window.location.hostname === "turkiyebilisim.com.tr" ||
      window.location.hostname === "www.turkiyebilisim.com.tr";

    if (!isProd) {
      window.alert(`${lang.label} dil desteği yalnızca canlı sitede (turkiyebilisim.com.tr) çalışır.`);
      return;
    }
  ...
  return (
    <div ref={ref} className="relative" style={{ zIndex: 1001 }}>
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 hover:border-[#06B6D4]/30 transition-all text-sm text-gray-300 hover:text-white cursor-pointer"
      >
          ...
          {languages.map((lang) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left hover:bg-white/5 transition-colors cursor-pointer ${
                current.code === lang.code ? "text-[#06B6D4]" : "text-gray-300"
              }`}
            >
