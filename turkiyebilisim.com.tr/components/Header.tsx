    ...
    <>
      ...
      <header
        className={`fixed top-0 left-0 right-0 z-[999] transition-colors duration-300 ${
          scrolled
            ? "border-b border-white/10 bg-[#08111d]/88 shadow-[0_18px_45px_rgba(2,8,23,0.35)] backdrop-blur-xl"
            : "bg-[#08111d]/72 backdrop-blur-md"
        }`}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <nav ref={navRef} className="hidden items-center gap-1 rounded-full border border-white/10 bg-white/[0.03] px-2 py-2 lg:flex">
              <a href={homeHref}
                className="whitespace-nowrap rounded-full px-4 py-2 text-sm font-medium text-gray-300 transition-colors hover:bg-white/5 hover:text-[#06B6D4]">
                {isEn ? "Home" : "Anasayfa"}
              </a>
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      aria-haspopup="true"
                      className={`flex cursor-pointer items-center gap-1 whitespace-nowrap rounded-full px-4 py-2 text-sm font-medium transition-colors ${
                        isActive ? "bg-white/5 text-[#06B6D4]" : "text-gray-300 hover:bg-white/5 hover:text-[#06B6D4]"
                      }`}
                    >
                      {menu.label}
                    ...
                    {isActive && (
                      <div
                        className="absolute top-full z-[1000] mt-3 rounded-[28px] border border-white/10 bg-[#0B1322]/95 p-5 shadow-2xl shadow-black/50 backdrop-blur-xl"
                        style={{
                          left: "50%",
                      ...
                      >
                        <a
                          href={menu.href}
                          className="mb-3 flex items-center gap-1 text-xs text-gray-500 transition-colors hover:text-[#06B6D4]"
                        >
                          {allLabel} {menu.label} →
                        </a>
                        <div className="grid gap-6" style={{ gridTemplateColumns: `repeat(${colCount}, 1fr)` }}>
                          {menu.cols.map((col) => (
                            <div key={col.title}>
                              <div className="text-xs font-bold text-[#06B6D4] tracking-widest uppercase mb-3 pb-2 border-b border-white/10">
                                {col.title}
                              </div>
                              <ul className="space-y-0.5">
                                {col.items.map((item) => (
                                  <li key={item.href}>
                                    <a href={item.href}
                                      className="group flex items-center gap-2 rounded-xl px-2 py-2 text-sm text-gray-300 transition-colors hover:bg-white/5 hover:text-[#06B6D4]">
              ...
              <a href={contactHref}
                className="ml-2 whitespace-nowrap rounded-full bg-[#06B6D4] px-5 py-2.5 text-sm font-bold text-black transition-all hover:-translate-y-0.5 hover:bg-[#22D3EE]">
      ...
      <div className="mobile-nav-css">
        <div style={{ padding: "16px 20px" }}>
          ...
          {mobileGroups.map((menu) => (
            <div key={menu.label}>
              <a href={menu.href} style={{ display: "block", padding: "14px 0", color: "#06B6D4", fontWeight: 700, fontSize: 16, borderBottom: "1px solid rgba(255,255,255,0.06)", textDecoration: "none" }}>
                {menu.label}
              </a>
          ...
          <a href={contactHref}
            style={{ display: "block", marginTop: 20, padding: "14px", background: "#06B6D4", color: "#000", fontWeight: 700, textAlign: "center", borderRadius: 12, textDecoration: "none", fontSize: 16 }}>
