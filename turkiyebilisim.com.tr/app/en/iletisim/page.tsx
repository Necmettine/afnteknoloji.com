...
export const metadata = {
  alternates: {
    canonical: "https://turkiyebilisim.com.tr/en/iletisim",
  }, title: "Contact | Türkiye Bilişim" };
      ...
      <section className="pt-32 pb-20 bg-[#0A0E1A] min-h-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="text-[#06B6D4] text-sm font-semibold tracking-widest uppercase">Contact us</span>
              ...
              {contacts.map((c) => (
                <div key={c.title} className="gradient-border rounded-2xl p-6 flex gap-4">
                  <div className="w-12 h-12 rounded-xl bg-[#06B6D4]/10 flex items-center justify-center flex-shrink-0">
                    <c.icon className="w-6 h-6 text-[#06B6D4]" />
                  </div>
                  <div>
                    <div className="font-semibold text-white mb-1">{c.title}</div>
                    {c.lines.map((l, i) =>
                      c.href && i === 0 ? (
                        <a key={i} href={c.href} className="block text-gray-400 hover:text-[#06B6D4] transition-colors text-sm">
                          {l}
                        </a>
                      ) : (
          ...
          <div className="mt-12">
            ...
            <div className="rounded-2xl overflow-hidden border border-white/10" style={{ height: 400 }}>
              <iframe
                ...
                title="Türkiye Bilişim Location"
              />
            </div>
          </div>
        </div>
